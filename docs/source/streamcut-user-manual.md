...
1. Welcome
2. How StreamCut works (the big picture)
3. Key words explained
4. What you need
5. Installing StreamCut
...
11. Analysis
12. The Clip Browser
13. Style Profiles (teaching StreamCut a look)
...
20. Publish Prep
21. Game guides
22. How StreamCut learns from you
...
StreamCut is your personal assistant editor. It watches your streams and recordings, finds the best moments, cuts out the boring parts, adds effects in the style you like, and prepares videos for YouTube.
...
StreamCut does most of the work, but **you stay in charge**. It always shows you its plan before making a final video, and you can change anything.
...
2. **Import** your footage into StreamCut: a Twitch VOD link or a local OBS recording.
3. **Analysis** runs once per recording. StreamCut transcribes what you say, listens for laughter, shouting, and gunfire, reads chat activity, and scores every moment.
...
6. StreamCut builds an **Edit Plan** and shows it on the **Review screen**.
...
9. Every change you make during review teaches StreamCut your taste for next time.
...
| **Analysis** | StreamCut studying a recording to find moments. Happens once per recording. |
| **Clip** | A short moment StreamCut found, like a funny death or a clutch fight. |
| **Clip Library** | The collection of all clips from all your recordings. |
| **Score** | How strong StreamCut thinks a moment is, from 0 (boring) to 1 (amazing). |
...
| **Edit Plan** | StreamCut's full plan for a video, before it's rendered. |
| **Part** | One episode of a Let's Play. |
| **Protected** | A section StreamCut won't cut, like a cutscene. |
| **Pinned** | A section you've told StreamCut to always keep. |
| **Trim level** | How aggressively StreamCut removes quiet or slow sections (Light, Standard, Tight). |
...
| **StreamCut** | The editor itself. The installer sets up everything else it needs (FFmpeg, AI runtime, AI models). |
| **Up-to-date NVIDIA drivers** | Required for AI and NVENC. |
| **DaVinci Resolve (free)** or **Kdenlive** *(optional)* | Only if you want to polish videos further after StreamCut. |
...
1. Download the StreamCut installer.
...
4. When installation finishes, open **StreamCut** from the Start menu.
...
Your Style Profiles, Rules, learned preferences, and Clip Library live in StreamCut's database. Back them up regularly:
...
StreamCut asks for five folders. Create them on your NVMe drive first, for example inside `D:\StreamCut\`.

| Folder | What goes in it | Example |
|---|---|---|
| **Raw footage** | Twitch VOD downloads and OBS recordings | `D:\StreamCut\raw` |
| **Working cache** | Temporary files during analysis | `D:\StreamCut\cache` |
| **Outputs** | Finished videos, exported timelines, thumbnails | `D:\StreamCut\output` |
| **Assets** | Your music, sound effects, fonts, overlay images | `D:\StreamCut\assets` |
| **Models** | AI model files | `D:\StreamCut\models` |
...
This lets StreamCut recognize *your* voice, especially on Twitch VODs where your voice is mixed with game sound and teammates.
...
A layout tells StreamCut where your facecam and gameplay are on screen. You make one per game (or per OBS scene).
...
StreamCut never adds music or sound effects you didn't provide.

---

## 7. Setting up OBS (step by step)

These settings are free and make a **big** difference to how good StreamCut's edits are. Menu names are from OBS Studio 30; they may differ slightly in other versions.

### 7.1 Why separate audio tracks matter

Normally a recording mixes your voice, game sound, and Discord into one sound. StreamCut can separate them using AI, but it's never perfect. If OBS records each sound on its own **track**, StreamCut knows exactly when *you* are talking, which improves captions, reaction detection, and clean cuts.
...
4. **Recording Path:** your StreamCut Raw footage folder.
...
9. Go to **Settings → Advanced → Recording** and tick **Automatically remux to mp4** if you prefer MP4 files. StreamCut works with both.
10. Click **Apply**.
...
5. Click **Show Connect Info** and copy the **password** into StreamCut's setup wizard.
6. Click **OK**.

### 7.5 Record while you stream (strongly recommended)

Twitch VODs only keep one mixed audio track and are lower quality. Recording locally at the same time gives StreamCut much better material. Your RTX 4070 Ti can stream and record at the same time.
...
If you do this, import the **local recording** into StreamCut and use the Twitch VOD only for chat (chapter 10.3).
...
**Always start Stream Companion before you go live or start recording.** If it wasn't running, StreamCut still works, but without markers and League events.

### 8.2 Using markers

- Press **Mark moment** (`Ctrl + Alt + M`) right after something great happens. You don't need to be precise; StreamCut looks at the moments leading up to your press.
...
Markers are the strongest signal StreamCut has. Even a few per stream noticeably improves highlights.
...
Right-click the tray icon → **View session log** to see markers and events from today's sessions. StreamCut matches these to your footage automatically during import.
...
**After the stream**
4. Open StreamCut → **Import** → add the local recording (and/or paste the Twitch VOD link for chat).
...
4. Stop recording.
5. In StreamCut, **Import** the recording, tag **The Blood of Dawnwalker**, and start analysis.
6. **Create video** → **Let's Play**. StreamCut proposes parts (usually about four 30-minute parts).
...
3. StreamCut checks the file and shows the audio tracks it found. Confirm which track is which (it remembers this for next time).
4. Choose the **game**.
...
2. In StreamCut, click **Import → Twitch VOD**.
3. Paste the link.
...
**Important:** Twitch deletes VODs after a limited time depending on your account type. StreamCut shows a warning for VODs close to expiring. Download VODs soon after streaming.

Because Twitch VODs have one mixed audio track, StreamCut automatically separates your voice from game sound and teammates using your voice sample. This adds some time to analysis.
...
3. StreamCut downloads only the chat and lines it up with your recording.

### 10.4 Games that show up in one session

If you switch games during a stream, open the recording in the Library, click **Game sections**, and mark where each game starts. StreamCut can suggest these automatically.
...
If your PC restarts or StreamCut closes during analysis, just reopen StreamCut and press **Resume**. Finished steps are not repeated.

---

## 12. The Clip Browser

The Clip Browser shows every moment StreamCut found.
...
- 👍 **Thumbs up** — "this is a good moment." StreamCut learns what you like.
...
Rating even 10–20 clips per week helps StreamCut learn fast.
...
6. StreamCut shows a **draft profile** with sliders and a list of techniques it noticed.
...
**Note:** StreamCut's measurements are estimates. Always check the sliders feel right.
...
Rules are tips you write in normal English. StreamCut turns each into a precise instruction and shows you how it understood it.
...
4. Click **Interpret**.
5. StreamCut shows its understanding in plain words, for example: *"In The Blood of Dawnwalker, protect any section tagged as a boss fight from being cut."*
...
When instructions disagree, StreamCut follows this order (top wins):

1. **Your rules**
2. Game defaults
3. Style Profile
4. What StreamCut learned from your feedback
5. General defaults

### 14.5 Rules StreamCut can't fully understand

If a tip doesn't match anything StreamCut can do precisely, it saves it as a **note**. The AI still reads notes when choosing clips, so they still help. Notes are shown with a 📝 icon.
...
### 15.3 How StreamCut decides where to split

StreamCut follows these rules, in order:
...
**How many parts will I get?** Usually about **four** from a 2-hour session. Sometimes three or five, because loading screens and menus are removed and story missions vary. StreamCut never adds boring footage just to make four parts. If you're regularly getting three, try **Trim level: Light** (default) or record a little longer.
...
- **Re-plan** — click to let StreamCut recalculate everything after your change.
...
2. StreamCut removes loading screens, menus, and dead air, speeds up travel, then removes the lowest-value sections until it reaches your target.
...
**If story content doesn't fit:** StreamCut won't secretly cut story. It will ask whether you want to switch to Split mode, use a longer target, or choose specific cutscenes to shorten.
...
Vertical clips are built entirely inside StreamCut and rendered here by default. The finished file is 1080×1920, the correct size for YouTube Shorts, TikTok, and Instagram Reels, so the same file works on all of them.
...
| Setting | Default | What it does |
|---|---|---|
| **Length** | 15–60 seconds | StreamCut picks the best length per clip within this range. Both platforms allow longer clips, but under 60 seconds tends to perform best. |
...
Each platform covers parts of the screen with its own buttons, username, and caption text. StreamCut keeps your captions and the action clear of those areas.
...
Every change you make here teaches StreamCut (chapter 22).

### 18.4 Saving and coming back

Click **Save plan** to keep your work. Find saved plans under **Projects**. You can close StreamCut and continue later.
...
Vertical clips are composed inside StreamCut: the crop, the stacked facecam, and the captions are all part of the plan. Those layout effects don't transfer reliably into other editors, so:

- **Recommended:** render Shorts and TikToks directly in StreamCut. They come out as finished files, ready to upload.
- If you do export a vertical project, StreamCut automatically pre-renders the vertical segments onto a 1080×1920 timeline, so Resolve or Kdenlive shows them exactly as designed. You can add extra polish there, but the layout is already baked in.
...
StreamCut suggests 3–5 titles. Click one to copy it, or edit it. For Let's Plays, titles follow your series format, for example *"The Blood of Dawnwalker — Part 7: [episode summary]"*.
...
StreamCut writes chapter timestamps. YouTube requires the first chapter to start at `0:00`, at least three chapters, and each chapter at least 10 seconds long. StreamCut follows these rules automatically.

### 20.4 Thumbnails

StreamCut exports several frames with your strongest reactions and most exciting moments as PNG images in your Outputs folder. Open them in a free tool such as GIMP, Photopea, or Canva's free plan to add text and design your thumbnail.
...
For Let's Plays, **Series → Release plan** lists parts in order. Add your planned upload dates to keep a steady schedule. StreamCut does not upload for you.
...
- Turn on in-game subtitles. They help StreamCut detect dialogue, and captions are placed so they don't cover them.
...
**Tips**
- Always run **Stream Companion** during League. Without it, StreamCut falls back to reading the screen, which is less accurate.
...
**Tips**
- StreamCut only looks at your recording. It never touches the game, so it's safe with BattlEye.
...
**Tips**
- Wardogs is in early access, so its screens change with updates. If kill feed or match result detection stops working after a game update, go to **Settings → Game profiles → Wardogs → Detection** and turn off screen reading until StreamCut is updated. Audio detection keeps working.
...
StreamCut doesn't retrain AI. Instead it quietly remembers your decisions and adjusts.
...
- **Effect suggestions** — if you keep removing an effect, StreamCut asks whether to use it less. Example: *"You've removed 8 zooms this week. Lower zoom frequency in 'Tarkov Hype Highlights'?"* Click **Yes**, **No**, or **Ask me later**.

### 22.3 Viewing and resetting

- **Learning → Summary** shows what StreamCut has learned in plain language, for example *"You prefer funny deaths over long fights in Tarkov."*
...
StreamCut warns you before starting a job if free space is low (default warning below **100 GB**, adjustable in Settings).
...
**StreamCut never deletes your raw footage automatically.** Delete raw files yourself once you're sure you've uploaded everything you want.

### 23.3 Moving footage to another drive

If you move raw files, open the project and click **Relink media** to point StreamCut to the new location.
...
> **[Engineer: add an entry for every error message the app can show.]**

### Stream Companion says "OBS: Not connected"
- Make sure OBS is open.
- Check **Tools → WebSocket Server Settings** in OBS: server enabled, port matches StreamCut.
- Re-copy the password from **Show Connect Info**.
- Check Windows Firewall isn't blocking StreamCut (allow it on private networks).
...
- Check the Queue: StreamCut may be running voice separation (Twitch VODs take longer).
...
- Turn off **Screen reading** for Wardogs in Settings → Game profiles. Audio detection continues. Update StreamCut when a fix is available.
...
StreamCut is designed never to touch your games. It only reads your recordings, Twitch chat, OBS, and League's official local game data. See chapter 27.
...
**What if I don't like any of StreamCut's edits?**
...
**Can I make a Short from a moment StreamCut didn't find?**
...
### Your game accounts
- StreamCut never reads game memory, injects into games, or automates input.
- Escape from Tarkov (BattlEye), League of Legends (Vanguard), and Wardogs anti-cheat are not affected by StreamCut because it only works with your recordings.
...
- StreamCut never adds music you didn't provide.

### Reference videos
- Reference videos are only studied on your PC to learn a style. StreamCut never puts their footage into your videos.
...
| Written for | StreamCut, as described in *Stream Auto-Editor — Project Specification v2.0* |
